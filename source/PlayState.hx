...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (SONG.song.toLowerCase())
		{
			...
			case 'thorns':
				dialogue = CoolUtil.coolTextFile(Paths.txt('thorns/thornsDialogue'));
                                          case 'autograph':
				dialogue = CoolUtil.coolTextFile(Paths.txt('autograph/dialog'));
                                          case 'pencilbook':
				dialogue = CoolUtil.coolTextFile(Paths.txt('pencilbook/dialog'));
                                          case 'therapicht':
				dialogue = CoolUtil.coolTextFile(Paths.txt('therapicht/dialog'));
		}
		...
		switch(SONG.stage)
		{
			...
			case 'park':
			{
					...
					var bg:FlxSprite = new FlxSprite(-1000, -300).loadGraphic(Paths.image('JavierThings/sky'));
					bg.antialiasing = true;
					bg.scrollFactor.set(0.2, 0.2);
					bg.active = false;
					bg.setGraphicSize(Std.int(bg.width * 0.8));
					...
					upperBoppers = new FlxSprite(-240, -90);
					upperBoppers.frames = Paths.getSparrowAtlas('nothing');
					upperBoppers.animation.addByPrefix('bop', "Upper Crowd Bob", 24, false);
					upperBoppers.antialiasing = true;
					upperBoppers.scrollFactor.set(0.33, 0.33);
					upperBoppers.setGraphicSize(Std.int(upperBoppers.width * 0.85));
					...
					var bgEscalator:FlxSprite = new FlxSprite(-250, -94).loadGraphic(Paths.image('JavierThings/town'));
					bgEscalator.antialiasing = true;
					bgEscalator.scrollFactor.set(0.3, 0.3);
					bgEscalator.active = false;
					bgEscalator.setGraphicSize(Std.int(bgEscalator.width * 0.9));
					...
					var tree:FlxSprite = new FlxSprite(370, -250).loadGraphic(Paths.image('nothing'));
					tree.antialiasing = true;
					tree.scrollFactor.set(0.40, 0.40);
					add(tree);

					var fgSnow:FlxSprite = new FlxSprite(-395, 500).loadGraphic(Paths.image('JavierThings/flour'));
					fgSnow.active = false;
					fgSnow.antialiasing = true;
					add(fgSnow);

					bottomBoppers = new FlxSprite(-110, 140);
					...
					santa = new FlxSprite(-840, 150);
					santa.frames = Paths.getSparrowAtlas('nothing');
					santa.animation.addByPrefix('idle', 'santa idle in fear', 24, false);
					santa.antialiasing = true;
					if(FlxG.save.data.distractions){
						add(santa);
		...
		healthBar.createFilledBar(0xFF55ACE2, 0xFF66FF33);
		// healthBar
		add(healthBar);
	...
	*/
	...
	override function stepHit()
	{
        ...
        if (SONG.song.toLowerCase() == 'pencilbook')
		{
			if(curStep == 1054)
			{
				remove(dad);
				dad = new Character(133, 260, 'javier-demon');
				add(dad);
			}
		}
        if (SONG.song.toLowerCase() == 'therapicht')
		{
			if(curStep == 352)
			{
				remove(dad);
				dad = new Character(133, 300,'javier-rehabilited');
				add(dad);
			}
		}
